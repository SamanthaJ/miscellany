...
!function($){
  ...
  $.fn.tip_cards = function(options){
    ...
    function realignCards() {
      var windowHeight = (( $(window).height() - $(".tc_modal").height() ) / 2+$(window).scrollTop());
      var timeout = 100;

      $(".tc_fake1").addClass("tc_straighten").css({
        "position":"absolute",
        "top": windowHeight + 16 + "px",
    ...
    el.find(settings.triggerSelector).addClass("tc_click_target").click(function() {
      ...
      $(".tc_next").click(function() {
        if ($(".tc_card.active").next(".tc_card").length > 0 && !$(this).hasClass("animated")) {
          ...
          var content = "<div class='tc_inner_modal'>" + $($(".tc_card.active").removeClass("active").next(".tc_card").addClass("active").attr("href")).html() + "</div>";
